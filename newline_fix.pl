...
foreach my $file (@input_files) {
  if ( $file =~ m/$pattern/i ) {
    ...
    while (<INFILE>) {
      ...
      $_ =~ s/\s+/ /g;		# change multi-whitespace to single
      $_ =~ s/\"\,\"/\|/g; 	# change "," to |
      $_ =~ s/^.//g;            # trim starting char (hopefully ")
      $_ =~ s/\"$//;            # trim ending "
      @delimCount = /\|/g;      # count the number of pipes
      $delimCount = scalar(@delimCount);
      warn "delimCount=$delimCount\n";
      print OUTFILE $_."\n";
    }
    close(OUTFILE);
    close(INFILE);
